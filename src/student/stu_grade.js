document.addEventListener('DOMContentLoaded', () => {
    ...
    function calculateGradeLevel(score) {
        if (score >= 90) return { grade: 'A', color: '#2196F3' };
        else if (score >= 80) return { grade: 'B', color: '#4CAF50' };
        else if (score >= 70) return { grade: 'C', color: '#FFC107' };
        else if (score >= 60) return { grade: 'D', color: '#FF9800' };
        else return { grade: 'F', color: '#F44336' };
    }
    ...
    async function displayAverageGrades() {
        try {
            ...
            const averageGradesData = await Promise.all(courses.map(async (course) => {
                ...
                // 출석 상태에 따른 점수 조정
                const adjustedGrades = validGrades.map(g => ({
                    blinkScore: g.attendance === 'absent' ? 0 : 
                               g.attendance === 'late' ? g.blinkScore * 0.7 : 
                               g.blinkScore,
                    concentrationScore: g.attendance === 'absent' ? 0 : 
                                      g.attendance === 'late' ? g.concentrationScore * 0.7 : 
                                      g.concentrationScore,
                    headPositionScore: g.attendance === 'absent' ? 0 : 
                                     g.attendance === 'late' ? g.headPositionScore * 0.7 : 
                                     g.headPositionScore,
                    total: g.attendance === 'absent' ? 0 : 
                           g.attendance === 'late' ? g.total * 0.7 : 
                           g.total
                }));

                const averages = {
                    blinkScore: average(adjustedGrades.map(g => g.blinkScore)),
                    concentrationScore: average(adjustedGrades.map(g => g.concentrationScore)),
                    headPositionScore: average(adjustedGrades.map(g => g.headPositionScore)),
                    total: average(adjustedGrades.map(g => g.total))
                };
    ...
    async function fetchCourseGrades() {
        try {
            ...
            const averageGradesData = await Promise.all(courses.map(async (course) => {
                ...
                // 출석 점수 계산 (집중도 total의 평균)
                const attendance_score = average(adjustedGrades.map(g => g.total));
            ...
            // 각 과목의 성적 데이터 생성
            const gradesData = averageGradesData.map(course => {
                // 중간고사, 기말고사, 과제 점수는 60-100 사이의 랜덤값 생성
                const midterm = Math.floor(Math.random() * (100 - 60 + 1)) + 60;
                const final = Math.floor(Math.random() * (100 - 60 + 1)) + 60;
                ...
                const total_score = (
                    ...
                    course.attendance_score * 0.2  // 출석(집중도) 20%
                );

                return {
                    course_name: course.c_name || course.name,
                    professor_name: course.professor_name || course.professorName,
                    midterm,
                    final,
                    assignment,
                    attendance_score: Math.round(course.attendance_score * 10) / 10,
                    total_score: Math.round(total_score * 10) / 10
                };
            });

            console.log("최종 성적 데이터:", gradesData);
            return gradesData;
        } catch (error) {
            console.error('성적 데이터 조회 오류:', error);
            return null;
        ...
        try {
            ...
            grades.forEach(course => {
                ...
                row.innerHTML = `
                    ...
                    <td><span class="score-value">${course.midterm}</span> <span class="score-total">/ 100</span></td>
                    <td><span class="score-value">${course.final}</span> <span class="score-total">/ 100</span></td>
                    <td><span class="score-value">${course.assignment}</span> <span class="score-total">/ 100</span></td>
                    <td><span class="score-value">${course.attendance_score}</span> <span class="score-total">/ 100</span></td>
    ...
    async function updateAttendanceStats() {
        try {
            const response = await fetch(`${SERVER_URL}/api/student/attendance/${currentUser.id}`);
            if (!response.ok) {
            ...
            data.forEach(record => {
                ...
                switch(status) {
                    ...
                    case '결석':
                        ...
                        break;
                    default:
                        console.log('알 수 없는 출석 상태:', status);
                }
            });

            console.log('집계된 통계:', stats);

            // UI 업데이트
            const updateElement = (type) => {
                const element = document.querySelector(`.stat-item .value[data-type="${type}"]`);
                if (element) {
                    element.textContent = `${stats[type]}회`;
                } else {
                    console.error(`${type} 통계 요소를 찾을 수 없습니다.`);
                }
            };

            // 각 상태별 UI 업데이트 (pending 제외)
            ['present', 'late', 'absent'].forEach(updateElement);
