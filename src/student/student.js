document.addEventListener('DOMContentLoaded', () => {
  ...
  async function initializeStudentMonitoring() {
    ...
    async function updateMonitoringUI(data) {
        if (data.type === 'final') {
            ...
            try {
                ...
                try {
                    ...
                    currentClass = currentAndNextCourse.currentClass;
                    console.log('현재 수업 정보:', currentClass);
                } catch (error) {
                    console.error('현재 수업 정보 가져오기 실패:', error);
                }
                ...
                if (currentClass) {
                    try {
                        // 현재 시간 (종료 시간)
                        const now = new Date();
                        const outTime = now.toLocaleTimeString('ko-KR', { 
                            hour: '2-digit', 
                            minute: '2-digit',
                            hour12: false
                        });
                        ...
                        // 저장된 버튼 클릭 시각 가져오기
                        const inTime = localStorage.getItem('monitoringClickTime');
                        if (!inTime) {
                            throw new Error('출석 시작 시각을 찾을 수 없습니다.');
                        }

                        // 총 시간 계산 (분 단위)
                        const startDate = new Date();
                        const [startHours, startMinutes] = inTime.split(':');
                        startDate.setHours(parseInt(startHours), parseInt(startMinutes), 0);
                        const totalMinutes = Math.round((now - startDate) / (1000 * 60));
                        ...
                        const [attendanceResponse, concentrationResponse] = await Promise.all([
                            fetch(`${SERVER_URL}/api/save_attendance`, {
                                ...
                                body: new URLSearchParams({
                                    m_id: currentUser.id,
                                    c_id: String(currentClass.c_id),
                                    in_time: inTime,
                                    out_time: outTime,
                                    t_total: `${totalMinutes}분`
                                }).toString()
                            }),
                            // 집중도 점수 저장
                        ...
                        const [attendanceResult, concentrationResult] = await Promise.all([
                        ...
                        console.log('서버 응답:', {
                            출석: attendanceResult,
                            집중도: concentrationResult
                        });

                        if (!attendanceResponse.ok || !concentrationResult.success) {
                            throw new Error(
                    ...
                    console.warn('현재 진행 중인 수업을 찾을 수 없습니다.');
                }
            } catch (error) {
                console.error('최종 결과 처리 중 오류:', error);
            }
            return;
        }        
        ...
        // 얼굴 인식 상태 표시
        if (webcamContainer) {
            webcamContainer.classList.toggle('face-detected', data.direction !== 'none');
            webcamContainer.classList.toggle('face-not-detected', data.direction === 'none');
        }
    }
