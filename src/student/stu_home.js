document.addEventListener('DOMContentLoaded', async () => {
  ...
  // 이벤트 리스너들을 한곳에서 관리
  let documentClickHandler; // 전역 범위에서 핸들러 선언

  function setupEventListeners() {
    // 다른 이벤트 리스너만 남기고 수강신청/취소 관련 리스너는 제거
  }

  // 모든 데이터 새로고침 함수
  async function refreshAllData() {
    try {
        // 전체 강의 목록 새로고침 (loadClasses 함수가 이미 이 작업을 수행)
        await loadClasses();
  ...
  function updateClassList(classes) {
    ...
    classes.forEach(cls => {
        ...
        row.innerHTML = `
            ...
            <td>${cls.m_name}</td>
            <td>${cls.class_day} ${cls.st_time}-${cls.end_time}</td>
            <td>${cls.current_count}/${cls.fixed_num}</td>
            <td><button class="btn btn-small register-btn">신청</button></td>
        `;
  ...
  async function registerCourse(courseId) {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.detail || '수강신청에 실패했습니다.');
      }

      alert('수강신청이 완료되었습니다.');
      await refreshAllData();  // 모든 데이터 새로고침
    } catch (error) {
  ...
  // 초기화 실행 (이것만 남기고 다른 초기 로드 코드는 제거)
  await initialize();
});