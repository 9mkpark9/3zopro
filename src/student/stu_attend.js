document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchEnrolledCourses() {
        try {
            const response = await fetch(`${SERVER_URL}/api/grades/courses/${currentUser.id}`);
            if (!response.ok) {
    ...
    async function updateAttendanceTable(courseId) {
        try {
            const response = await fetch(`${SERVER_URL}/attendance/student/${currentUser.id}/${courseId}`);
            if (!response.ok) {
