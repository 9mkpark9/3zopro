document.addEventListener('DOMContentLoaded', () => {
  ...
  if (chartArea) {  // chart-area 요소가 존재하는 경우에만 차트 초기화
        ...
        canvas.toBlob(async (blob) => {
            try {
                ...
                reader.onloadend = async () => {
                    ...
                    // Python 프로세스로 프레임 데이터 전송
                    const response = await window.electron.invoke('process-frame', {
                        frame: base64data
                    });

                    if (response.success) {
                        drawDetections(ctx, response.detections);
                        updateTrackingInfo(response.detections);
                    }
                };
            } catch (error) {
  ...
  function drawDetections(ctx, detections) {
    detections.forEach(detection => {
        ...
        // 바운딩 박스 그리기
        ctx.strokeStyle = detection.matched ? '#00bf9a' : '#fd5d93';
        ctx.lineWidth = 2;
        ...
        ctx.fillStyle = 'white';
        ctx.font = '12px Arial';
        ctx.fillText(`ID: ${detection.id}`, x, y - 5);
    });
  }

  // 트래킹 정보 업데이트
  function updateTrackingInfo(detections) {
    const memberList = document.getElementById('modalMemberList');
    if (!memberList) return;

    const presentMembers = detections.filter(d => d.matched);
    const absentMembers = detections.filter(d => !d.matched);

    // 멤버 목록 업데이트
    memberList.innerHTML = `
        <div class="attendance-summary">
            <p>출석: ${presentMembers.length}명</p>
            <p>미출석: ${absentMembers.length}명</p>
        </div>
        ${detections.map(detection => `
            <div class="member-item ${detection.matched ? 'present' : 'absent'}">
                <div class="member-info">
                    <span class="material-icons">person</span>
                    <div class="info">
                        <span class="name">ID: ${detection.id}</span>
                        <span class="status">${detection.matched ? '출석' : '미출석'}</span>
                    </div>
                </div>
            </div>
        `).join('')}
...
class ReIDManager {
    ...
    async startRecognition() {
        if (this.isElectron) {
            return await window.electron.invoke('start-reid');
        } else {
    ...
    async stopRecognition() {
        if (this.isElectron) {
            return await window.electron.invoke('stop-reid');
        } else {
    ...
    async processFrame(frameData) {
        if (this.isElectron) {
            return await window.electron.invoke('process-frame', {
                frame: frameData
            });
        } else {
...
async function startLecture() {
    if (isLectureActive) return;
    
    try {
        const response = await reidManager.startRecognition();
        if (response.success) {
            isLectureActive = true;
            updateUIForActiveClass();
            console.log('ReID system started successfully');
        } else {
            console.error('Failed to start ReID:', response.error);
            alert('강의 시작에 실패했습니다.');
        }
    } catch (error) {
...
async function endLecture() {
    ...
    try {
        // Python ReID 프로세스 종료
        const response = await window.electron.invoke('stop-reid');
        
        if (response.success) {
            isLectureActive = false;
            updateUIForInactiveClass();
            console.log('ReID process stopped successfully');
        } else {
            console.error('Failed to stop ReID process:', response.error);
            alert('강의 종료에 실패했습니다.');
        }
    } catch (error) {
